 ...
 */
CvPoint binarisation(IplImage *img, int *nbPixels, colorCalibrage *calibrage) {
    ...
    *nbPixels = 0;
    ...
    // We create the mask
    cvInRangeS(hsv, cvScalar(calibrage->iLowH, calibrage->iLowS, calibrage->iLowV), cvScalar(calibrage->iHighH, calibrage->iHighS, calibrage->iHighV), mask);
...
int main() {
    ...
    jeuCouleurs def = defaut();
    colorCalibrage calibrage = def.rouge;
    ...
    // Check if the capture is ok
        if (!capture) {
        cout << "Can't initialize the video capture." << endl;
            return -1;
    }
    ...
    while(key != 'Q' && key != 'q') {
        ...
        objectNextPos = binarisation(image, &nbPixels, &calibrage);
